...
model = Surrogate_GNN(n_node, attrs.shape[1], train_idx, test_idx, args.device)
...
#%%
RWLS_4 = np.loadtxt(dirs+'/data/'+args.dataset+'/RWLS_greedy_lam=0.1/'+str(args.trial)+'/RWLS_0.1_surrogate_auc.txt')
RWLS_4 = np.concatenate((np.array([0.83]).reshape(1,1), RWLS_4.reshape(-1,1)))
#%%
plt.title('BlogCatalog')
plt.plot(ckt[:,0], ckt[:,2], label='BinarizedAttack')
plt.plot(ckt[:,0], RWLS_4[ckt[:,0].astype('int32')], label='GradMaxSearch')
plt.legend()
plt.xlabel('B')
plt.ylabel('Surrogate Testing AUC')
plt.show()
#%%
def train(epochs, lr):
